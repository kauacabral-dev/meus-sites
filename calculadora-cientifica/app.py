...
def calcular():
    ...
    if escolha == 'funcaoquadratica':
        ...
        if None in (a, b, c):
            ...
            graph_html = plot_func(x_vals, y_vals, title="Função Quadrática", highlight_points=highlight)
            r += f'<br>{graph_html}'
        ...
        if None in (a, b):
            ...
            if x_input is not None:
                highlight.append((x_input, fx, f'F({format_number(x_input)})={format_number(fx)}'))
            graph_html = plot_func(x_vals, y_vals, title="Função Afim", highlight_points=highlight)
            r += f'<br>{graph_html}'
        ...
        if None in (a, b):
            ...
            if x_input is not None:
                highlight.append((x_input, fx, f'F({format_number(x_input)})={format_number(fx)}'))
            graph_html = plot_func(x_vals, y_vals, title="Função Exponencial", highlight_points=highlight)
            r += f'<br>{graph_html}'
        ...
        if None in (a, b) or sinal == '':
            ...
            graph_html = plot_func(x_vals, y_vals, title="Inequação Linear", highlight_points=highlight, inequacao=inequacao_interval)
            r += f'<br>{graph_html}'
